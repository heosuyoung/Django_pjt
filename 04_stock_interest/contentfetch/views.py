...
from .models import Stock  # ✅ 관심 종목 모델
...
service = Service(ChromeDriverManager().install())  # ChromeDriver 자동 다운로드
    ...
    if request.method == "POST":
        ...
        if stock_name:
            obj, created = Stock.objects.get_or_create(
                user=request.user, name=stock_name
            )
            if not created:
                # 이미 존재할 경우 메시지 전달
                return render(
                    ...
                    "contentfetch/stock_finder.html",
                    {
                        ...
                        "is_existing_data": True,
                        "is_loading": False,
                        "error_message": "이미 관심 종목에 추가된 항목입니다.",
                    },
                )
